...
export class GDDInterventionReports extends connectStore(PaginationMixin(CommonMixin(EndpointsLitMixin(LitElement)))) {
  ...
  stateChanged(state: RootState) {
    if (EtoolsRouter.pageIsNotCurrentlyActive(get(state, 'app.routeDetails'), 'gpd-interventions', 'reports')) {
      return;
    }
  ...
  private updateCurrentParams(paramsToUpdate: GenericObject<any>, reset = false): void {
    ...
    history.pushState(window.history.state, '', `gpd-interventions/${this.interventionId}/reports?${stringParams}`);
